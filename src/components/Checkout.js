...
const Checkout = () => {
   ...
   */
  const Address = ({ list }) => {
    return list.all.map((ele) => {
      return (
        <Box
          className={
            list.selected === ele._id
        ...
        >
          <Typography color="#3C3C3C" alignSelf="center">
            {ele.address}
          </Typography>
          <Button
  ...
  const performCheckout = async (token, items, addresses) => {
    ...
    {
      let val=getTotalCartValue(items);
      let currbal=localStorage.getItem('balance');
      const response = await axios.post(
      ...
      {
        localStorage.setItem('balance',currbal-val);
      ...
      <Grid container>
        <Grid item xs={12} md={9}>
          <Box className="shipping-container" minHeight="100vh">
            ...
            <Button startIcon={<CreditCard />} variant="contained" onClick={()=>{performCheckout(token,items,addresses)}}>
              PLACE ORDER
            </Button>
          </Box>
        </Grid>
